import os,io,json,random,hashlib,logging,re
...
ILLEGAL_TAGS=["underage","minor","child","loli","shota","young","agegap","rape","sexual violence","bestiality","zoophilia","bestial","scat","fisting","incest","pedo","pedophile","creampie"]
...
def contains_illegal_indicators(text:str)->bool:
    ...
    for bad in ILLEGAL_TAGS:
        if bad in normalized: return True
    return False
...
if not os.path.exists(DATA_FILE):
    with open(DATA_FILE,"w") as f:
        json.dump({"join_counts":{},"used_gifs":{},"provider_weights":{},"sent_history":{},"gif_tags":[]},f,indent=2)
with open(DATA_FILE,"r") as f: data=json.load(f)
data.setdefault("join_counts",{}); data.setdefault("used_gifs",{}); data.setdefault("provider_weights",{}); data.setdefault("sent_history",{}); data.setdefault("gif_tags",[])
...
if not GIF_TAGS: GIF_TAGS=["hentai"]
    ...
    try:
        async with session.get(url,timeout=timeout,allow_redirects=True) as resp:
            ...
            async for chunk in resp.content.iter_chunked(1024):
                ...
                if total>size_limit:
                    if DEBUG_FETCH: logger.debug(f"download exceeded {size_limit} for {url}")
                    return None,ctype
...
PROVIDER_TERMS={
"waifu_pics":["hentai","ecchi","oral","anal","blowjob","cumshot","paizuri","oppai","milf","mature"],
"waifu_im":["hentai","ero","ecchi","milf","oral","oppai","cum"],
"waifu_it":["waifu","hentai","ero"],
...
def map_tag_for_provider(provider:str,tag:str)->str:
    t=(tag or "").lower().strip(); pool=PROVIDER_TERMS.get(provider,[])
    if t:
        for p in pool:
            if p in t: return p
...
async def fetch_from_waifu_im(session,positive):
    try:
        ...
        headers={}
        if WAIFUIM_API_KEY: headers["Authorization"]=f"Bearer {WAIFUIM_API_KEY}"
...
async def fetch_from_danbooru(session,positive):
    try:
        ...
        auth=None
        if DANBOORU_USER and DANBOORU_API_KEY: auth=aiohttp.BasicAuth(DANBOORU_USER,DANBOORU_API_KEY)
...
PROVIDER_FETCHERS={
"waifu_pics":fetch_from_waifu_pics,"waifu_im":fetch_from_waifu_im,"waifu_it":fetch_from_waifu_it,
"nekos_best":fetch_from_nekos_best,"nekos_life":fetch_from_nekos_life,"nekos_moe":fetch_from_nekos_moe,
"nekoapi":fetch_from_nekos_moe,"otakugifs":fetch_from_otakugifs,"animegirls_online":fetch_from_animegirls_online,
"nekos_api":fetch_from_nekos_api,"danbooru":fetch_from_danbooru,"gelbooru":fetch_from_gelbooru,
"konachan":fetch_from_konachan,"rule34":fetch_from_rule34
}

_provider_cycle_deque=deque(); _last_cycle_refresh=None
def build_provider_pool():
    ...
    if TRUE_RANDOM:
        random.shuffle(available); return available
    ...
    if not _provider_cycle_deque or (_last_cycle_refresh and (now-_last_cycle_refresh)>timedelta(minutes=15)):
        random.shuffle(available); _provider_cycle_deque=deque(available); _last_cycle_refresh=now
...
async def attempt_get_media_bytes(session,gif_url):
    ...
    if contains_illegal_indicators(gif_url): return None,None,"illegal-indicator-in-url"
    ...
    if status is None:
        ...
        if b: return b,ctype,"downloaded-after-head-failed"
        return None,ctype,"head-failed-get-failed"
    if status not in (200,301,302):
        b,ctype=await _download_bytes_with_limit(session,gif_url,size_limit=HEAD_SIZE_LIMIT)
        if b: return b,ctype,f"get-after-head-{status}"
    ...
    if cl:
        try:
            clv=int(cl)
            if clv>HEAD_SIZE_LIMIT: return None,ctype,f"too-large-head-{clv}"
            b,ctype2=await _download_bytes_with_limit(session,gif_url,size_limit=HEAD_SIZE_LIMIT)
            if b: return b,ctype2 or ctype,"downloaded-with-head-size"
            return None,ctype2 or ctype,"head-said-small-but-get-failed"
        except Exception:
            b,ctype2=await _download_bytes_with_limit(session,gif_url,size_limit=HEAD_SIZE_LIMIT)
            if b: return b,ctype2 or ctype,"downloaded-with-head-parse-except"
            return None,ctype2 or ctype,"head-parse-get-failed"
    else:
        b,ctype2=await _download_bytes_with_limit(session,gif_url,size_limit=HEAD_SIZE_LIMIT)
        if b: return b,ctype2 or ctype,"downloaded-unknown-size"
...
def make_embed(title,desc,member,kind="join",count=None):
    ...
    if count: footer+=f" • Joins: {count}"
    embed.set_footer(text=footer); return embed
        ...
        if fmt.upper() in ("GIF","WEBP"):
            ...
            for scale_step in range(1,12):
                ...
                try:
                    ...
                    try: resized[0].save(out,format="GIF",save_all=True,append_images=resized[1:],loop=0)
                    except Exception: out=None
                if out and out.getbuffer().nbytes<=max_size: return out.getvalue()
            return None
        else:
            w,h=img.size
            for step in range(1,12):
                ...
                if fmt.upper() in ("JPEG","JPG"):
                    img2.save(out,format="JPEG",quality=max(10,95-6*step),optimize=True)
                else:
                    img2.save(out,format="PNG",optimize=True)
                if out.getbuffer().nbytes<=max_size: return out.getvalue()
            return None
    except Exception as e:
...
async def send_embed_with_media(text_channel,member,embed,gif_bytes,gif_name,gif_url,ctype=None):
    ...
    try:
        ...
        except Exception: logger.debug("final fallback failed")
...
intents=discord.Intents.default(); intents.guilds=True; intents.members=True; intents.voice_states=True; intents.message_content=True
...
async def ensure_connected_task():
    try:
        ...
        vc_id=VC_IDS[0]; channel=bot.get_channel(vc_id)
        if not channel:
            for g in bot.guilds:
        ...
        if not vc:
            try:
                ...
                if DEBUG_FETCH: logger.debug(f"connected to VC {vc_id}")
            except Exception as e:
                if DEBUG_FETCH: logger.debug(f"failed connect: {e}")
        else:
            if vc.channel.id!=channel.id:
                try: await vc.move_to(channel)
                except Exception as e:
                    if DEBUG_FETCH: logger.debug(f"move failed: {e}")
    except Exception as e:
        if DEBUG_FETCH: logger.debug(f"ensure_connected unexpected: {e}")

@bot.event
async def on_ready():
    try: autosave_task.start()
    except RuntimeError: pass
    try: ensure_connected_task.start()
    except RuntimeError: pass
    available=[]
...
async def on_voice_state_update(member,before,after):
    ...
    if before.channel and (before.channel.id in VC_IDS) and (after.channel!=before.channel):
        raw=random.choice(LEAVE_GREETINGS); msg=raw.format(display_name=member.display_name)
        embed=make_embed("Goodbye!",msg,member,"leave")
        gif_bytes,gif_name,gif_url,ctype=await fetch_gif(member.id)
        await send_embed_with_media(text_channel,member,embed,gif_bytes,gif_name,gif_url,ctype)

if __name__=="__main__":
    if not TOKEN: logger.error("TOKEN missing. Set TOKEN and restart.")
    else: bot.run(TOKEN)
